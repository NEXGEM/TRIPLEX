...
def load_callbacks(cfg: Dict):
    ...
    if mode == 'max':
        ...
        checkpoint_callback = ModelCheckpoint(monitor = target,
                                        ...
                                        every_n_epochs = 1 if save_best_only else 10,
                                        mode = mode,
                                        save_weights_only = True)
...
def load_config_with_default(config_path, default_config_path=None):
    """Load config with default config support."""
    
    if default_config_path is None:
        default_config_path = default_config_path = '/'.join(config_path.split('/')[:-1])
        default_config_path = f"{default_config_path}/default.yaml"
    
    if default_config_path and os.path.exists(default_config_path):
        default_cfg = load_config(default_config_path)
        cfg = load_config(config_path)
        
        # Recursively update default config with specific config
        def update_dict(default_dict, new_dict):
            for key, value in new_dict.items():
                if key in default_dict and isinstance(default_dict[key], dict) and isinstance(value, dict):
                    update_dict(default_dict[key], value)
                else:
                    default_dict[key] = value
        
        update_dict(default_cfg, cfg)
        return default_cfg
    else:
